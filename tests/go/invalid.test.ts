...
describe('Go Invalid Files', () => {
  ...
  test('undefined-function.go should exit with code 2 and show exact error', async () => {
    const filePath =
      'tests/fixtures/go/invalid/undefined-function/undefined-function.go';
    const proc = await runDiagnostics(filePath);
    expect(proc.exitCode).toBe(2);
    expect(stripAnsi(proc.stderr.toString())).toBe(
      `[compiler] ERROR at line 9, column 2: undefined: undefinedFunction [UndeclaredName]`
    );
  });
  ...
  test('syntax-error.go should exit with code 2 and show exact error', async () => {
    ...
    expect(proc.exitCode).toBe(2);
    expect(stripAnsi(proc.stderr.toString())).toBe(
      `[syntax] ERROR at line 7, column 15: missing ',' before newline in argument list`
    );
  });
});
