...
export async function formatHoverResults(results: HoverResult[]): Promise<string> {
  if (results.length === 0) {
  ...
  for (const result of results) {
    ...
    output.push(`${CYAN}Location:${RESET_COLOR} ${result.location.file}:${result.location.line + 1}:${result.location.column + 1}\n${content}`);
